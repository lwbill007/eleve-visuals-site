...
export default function FinancialCenterPage() {
  ...
  async function addManual() {
    ...
    if (res.ok) {
      toast("Manual payment recorded as pending reconciliation.");
      setAmount("");
      setEmail("");
      setDesc("");
      void load();
  ...
  async function reconcile(paymentId: string, verificationStatus: "verified" | "rejected") {
    const res = await adminFetch("/api/admin/payments", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ paymentId, verificationStatus }),
    });
    if (!res.ok) {
      toast("Could not reconcile payment.", "error");
      return;
    }
    toast(verificationStatus === "verified" ? "Payment reconciled." : "Payment rejected.");
    void load();
  }
  ...
  return (
    <AdminShell title="Financial Center">
      <WorkspaceChrome
        ...
        description="Owns Revenue for ÉLEVÉ OS. Only Stripe-confirmed or reconciled manual payments count as verified settled revenue."
        onRefresh={() => void load()}
        refreshing={loading}
        related={[
      ...
      >
        {loading && !summary ? (
          ...
          <div className="space-y-10">
            ...
            {(summary?.pendingManualCount ?? 0) > 0 ? (
              <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-fog">
                {summary!.pendingManualCount} manual payment{summary!.pendingManualCount === 1 ? "" : "s"} · $
                {summary!.pendingManual.toLocaleString()} pending reconciliation and excluded from verified revenue.
              </div>
            ) : null}
            ...
            <AdminPanel
              title="Manual deposit / payment"
              subtitle="Pending evidence until an admin reconciles it"
            >
            ...
            <AdminPanel
              ...
              subtitle={
                filtered.length > 0
                  ? `${filtered.length} shown · verification shown per row`
                  : summary?.hasPayments
            ...
            >
              ...
              {filtered.length === 0 ? (
                <WorkspaceEmpty
                  ...
                  detail="Connect Stripe webhooks or record manual evidence for reconciliation."
                  actionHref="/admin/qa"
                  ...
                  {filtered.map((p) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {p.source === "manual" && p.verificationStatus === "pending" ? (
                          <>
                            <WorkspaceButton
                              variant="secondary"
                              onClick={() => void reconcile(p.id, "verified")}
                            >
                              Reconcile
                            </WorkspaceButton>
                            <WorkspaceButton
                              variant="ghost"
                              onClick={() => void reconcile(p.id, "rejected")}
                            >
                              Reject
                            </WorkspaceButton>
                          </>
