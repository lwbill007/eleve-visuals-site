...
import { emailComposerHref } from "@/lib/admin-operations";
  ...
  async function updateStatus(next: string) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.error || "Could not update stage.");
      }
      setStatus(next);
      toast("Stage updated.");
    } catch (statusError) {
      toast(
        statusError instanceof Error ? statusError.message : "Could not update stage.",
        "error"
      );
    }
  }
