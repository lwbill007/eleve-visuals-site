...
export default function WorkboardPage() {
  ...
  const openInquiries = staleFirst(
    columns
  ...
  const inbox = openInquiries.slice(0, 12);
  ...
  const production = columns
    .filter((c) => ["booked", "planning", "production", "editing"].includes(c.id))
    .flatMap((c) => c.items.map((i) => ({ ...i, stage: c.label })));
  ...
  const lanes: OsCapability[] = [
    {
      id: "tasks",
      label: "Tasks",
      status: openInquiries.length > 0 ? "live" : "partial",
      summary: `${openInquiries.length} open inquiries need action (Measured · Pipeline).`,
      href: "/admin/pipeline",
    },
    {
      id: "shoots",
