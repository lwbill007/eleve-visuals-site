...
export default function AdminNotificationsPage() {
  ...
  const [activeView, setActiveView] = useState<"inbox" | "delivery" | "configuration">("inbox");
  ...
  return (
    <AdminShell title="Notifications">
      ...
      <div
        ...
        className="mb-8 flex gap-1 overflow-x-auto border-b border-stone/25"
      >
        {(["inbox", "delivery", "configuration"] as const).map((view) => (
          <button
            key={view}
            type="button"
            aria-pressed={activeView === view}
            onClick={() => setActiveView(view)}
            className={`min-h-11 shrink-0 border-b-2 px-4 text-xs tracking-[0.12em] uppercase ${
              activeView === view
      ...
      {activeView === "delivery" && analytics ? (
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard
            ...
            value={
              analytics.delivery.successRate == null
                ? "Not observed"
                : `${analytics.delivery.successRate}%`
            }
            hint={
              analytics.delivery.successRate == null
                ? "No attempted deliveries in the last 30 days"
                : `${analytics.delivery.sent} sent · ${analytics.delivery.skipped} skipped (30d)`
            }
          />
          <MetricCard
          ...
          <MetricCard
            ...
            value={analytics.avgReadMs == null ? "Not observed" : formatDuration(analytics.avgReadMs)}
            hint={`${analytics.readCount} submissions read (30d)`}
          />
        </div>
      ) : null}

      {activeView === "delivery" && health ? (
      ...
      <div className="space-y-8">
        <section className={activeView === "configuration" ? "space-y-4" : "hidden"}>
        ...
        <section
          className={
            activeView === "configuration" ? "grid gap-6 sm:grid-cols-2" : "hidden"
          }
        >
          <StringListEditor
        ...
        <section className={activeView === "configuration" ? "space-y-3" : "hidden"}>
        ...
        <section className={activeView === "delivery" ? "space-y-4" : "hidden"}>
      ...
      <OsCapabilityGrid
        ...
        subtitle="Measured channel status and explicitly unavailable alert SLAs."
