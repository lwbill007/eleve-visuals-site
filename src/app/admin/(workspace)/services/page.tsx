...
export default function AdminServicesPage() {
  ...
  async function save() {
    ...
    if (!editing.title?.trim()) errors.title = "Service title is required.";
    if (!editing.slug?.trim()) errors.slug = "A URL slug is required.";
    if (editing.image && !editing.imageAlt?.trim()) {
      errors.imageAlt = "Add alt text for the service image.";
    }
  ...
  async function remove(item: ServiceDTO) {
    if (!confirmNamedDestructive("service", item.title)) return;
    const res = await adminFetch(`/api/admin/services/${item.id}`, { method: "DELETE" });
        ...
        {editing && (
          <div className="min-w-0 border border-stone/30 p-6 lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="space-y-4">
              ...
              <AdminField label="Slug" error={validationErrors.slug}>
                <AdminInput
                  value={editing.slug || ""}
              ...
              />
              <AdminField label="Service Image Alt Text" error={validationErrors.imageAlt}>
                <AdminInput
              ...
              >
                {uploadsActive ? "Wait for uploads…" : saving ? "Saving..." : "Save Service"}
              </button>
            </div>
          </div>
        )}
