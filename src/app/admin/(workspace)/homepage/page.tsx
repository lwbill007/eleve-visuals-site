...
export default function AdminHomepagePage() {
  ...
  const editor = useAdminEditor(
    ...
    async (next) => {
      const results = await Promise.all([
        saveAdminContent("hero", next.hero),
        saveAdminContent("homepage", next.homepage),
      ]);
  ...
  useEffect(() => {
    adminFetch("/api/admin/content")
      ...
      .then((items: { key: string; value: unknown }[]) => {
        let nextHero = DEFAULT_HERO;
        let nextHomepage = DEFAULT_HOMEPAGE;
        for (const item of items) {
          if (item.key === "hero") {
            nextHero = { ...DEFAULT_HERO, ...(item.value as HeroContent) };
          }
        ...
        resetEditor({ hero: nextHero, homepage: nextHomepage });
      });
  ...
  async function handleSave() {
    ...
    setMessage(ok ? "Homepage saved." : "Some fields failed to save.");
  }
  ...
  return (
    <AdminShell title="Homepage Intelligence">
      ...
      <SaveBar
        ...
        message={message}
        autosaveNote={editor.dirty ? "Unsaved changes" : ""}
      />
      </WorkspaceChrome>
    </AdminShell>
  );
