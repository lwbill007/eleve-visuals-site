...
export default function ReferralsHubPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError("Could not load lead-source attribution from CRM.");
      setContacts([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AdminShell title="Lead Sources">
      <WorkspaceChrome
        eyebrow="Grow"
        title="Lead-source attribution"
        description="Booking-reported acquisition sources from CRM. These counts show attributed contacts, not verified referral conversions or campaign ROI."
        onRefresh={() => void load()}
        refreshing={loading}
        extra={
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {bySource.slice(0, 6).map(([source, count]) => (
                <AdminPanel key={source} title={source}>
                  ...
                  <p className="text-xs text-muted">contacts attributed to this source</p>
                </AdminPanel>
              ))}
              {bySource.length === 0 && (
                <WorkspaceEmpty
                  title="No lead sources yet"
                  detail="When bookings include a source field, attribution appears here. Capture source on the booking form."
