...
export default function AdminTestimonialsPage() {
  ...
  async function save() {
    ...
    if (!editing.quote?.trim()) errors.quote = "Testimonial quote is required.";
    if (!editing.name?.trim()) errors.name = "Client name is required.";
    if (editing.image && !editing.imageAlt?.trim()) {
      errors.imageAlt = "Add alt text for the client photo.";
    }
  ...
  async function remove(item: TestimonialDTO) {
    if (!confirmNamedDestructive("testimonial", item.name)) return;
    const res = await adminFetch(`/api/admin/testimonials/${item.id}`, { method: "DELETE" });
    if (!res.ok) {
      ...
      {editing && (
        <div className="mb-8 border border-stone/30 p-6">
          <div className="space-y-4">
            <AdminField label="Quote" error={validationErrors.quote}>
              <AdminTextarea
                value={editing.quote || ""}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <AdminField label="Name" error={validationErrors.name}>
                <AdminInput
                  value={editing.name || ""}
            ...
            />
            <AdminField label="Client Photo Alt Text" error={validationErrors.imageAlt}>
              <AdminInput
            ...
            <div className="flex gap-3">
              ...
              >
                {uploadsActive ? "Wait for uploads…" : saving ? "Saving..." : "Save"}
              </button>
          ...
          <div key={item.id} className="flex items-start gap-3 border border-stone/30 p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex gap-3">
                ...
                <button type="button" onClick={() => remove(item)} className="text-xs text-red-400">
                  Delete
                </button>
              </div>
            </div>
