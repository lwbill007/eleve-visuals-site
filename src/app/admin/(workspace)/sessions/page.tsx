...
export default function AdminSessionsPage() {
  ...
  async function save() {
    if (!editing || saving || uploadsActive) return;
    const errors: Record<string, string> = {};
    if (!editing.title?.trim()) errors.title = "Volume title is required.";
    if (!editing.slug?.trim()) errors.slug = "A URL slug is required.";
    if (editing.posterImage && !editing.posterImageAlt?.trim()) {
      errors.posterImageAlt = "Add alt text for the poster image.";
    }
    if (editing.bannerImage && !editing.bannerImageAlt?.trim()) {
      errors.bannerImageAlt = "Add alt text for the banner image.";
    }
  ...
  async function remove(item: SessionVolumeDTO) {
    if (!confirmNamedDestructive("session volume", item.title)) return;
    const res = await adminFetch(`/api/admin/session-volumes/${item.id}`, { method: "DELETE" });
    if (!res.ok) {
      ...
      {editing && (
        <div className="mb-10 border border-stone/30 p-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <AdminField label="Title" error={validationErrors.title}>
              <AdminInput
                value={editing.title || ""}
                onChange={(e) => update("title", e.target.value)}
              />
            </AdminField>
            <AdminField label="Slug" hint="URL: /sessions/your-slug" error={validationErrors.slug}>
              <AdminInput
                value={editing.slug || ""}
            ...
            <div className="md:col-span-2">
              ...
              />
              <div className="mt-4">
                <AdminField label="Poster Image Alt Text" error={validationErrors.posterImageAlt}>
                  <AdminInput
                    value={editing.posterImageAlt || ""}
                    onChange={(e) => update("posterImageAlt", e.target.value)}
                  />
                </AdminField>
              </div>
            </div>
            <div className="md:col-span-2">
              ...
              />
              <div className="mt-4">
                <AdminField label="Banner Image Alt Text" error={validationErrors.bannerImageAlt}>
                  <AdminInput
                    value={editing.bannerImageAlt || ""}
                    onChange={(e) => update("bannerImageAlt", e.target.value)}
                  />
                </AdminField>
              </div>
            </div>
            ...
            >
              ...
              <div className="flex w-full flex-wrap items-center gap-x-4 gap-y-2 sm:w-auto sm:justify-end">
                ...
                <button
                  ...
                  className="py-1 text-xs text-accent"
                >
                  Edit
                </button>
                ...
                <button type="button" onClick={() => remove(item)} className="py-1 text-xs text-red-400">
                  Delete
                </button>
              </div>
            </div>
