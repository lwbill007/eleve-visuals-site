...
export async function getAdminInsights() {
  const [crm, dashboard, analytics] = await Promise.all([
    getAdminCRMContacts(),
    getAdminDashboardOS(),
  ...
  const inactive = crm.filter((c) => {
    ...
    return days > 90 && c.status === "completed";
  });
  if (inactive.length > 0) {
    insights.push({
      ...
      detail: "Past clients with no activity in 90+ days. A re-engagement campaign could drive repeat bookings.",
      action: "Create re-engagement campaign",
      href: "/admin/email",
    });
  }

  if (dashboard.metrics.applications.pending > 5) {
    insights.push({
      id: "applications-backlog",
      severity: "medium",
      title: `${dashboard.metrics.applications.pending} applications awaiting review`,
      detail: "Fast responses improve acceptance rates and community perception.",
  ...
  if (dashboard.metrics.monthlyGrowth < 0) {
    insights.push({
      id: "bookings-down",
      severity: "high",
      title: `Bookings down ${Math.abs(dashboard.metrics.monthlyGrowth)}% vs last month`,
      detail: "Consider promoting your highest-converting portfolio and sending a newsletter to warm leads.",
  ...
  if (topPage && topPage.path.includes("/portfolio")) {
    insights.push({
      id: "portfolio-converts",
      severity: "low",
      title: `"${topPage.path}" is your top traffic driver`,
      detail: `${topPage.views} views this month. Feature this work on homepage and in email campaigns.`,
      action: "Edit homepage",
      href: "/admin/homepage",
    });
  }

  if (dashboard.metrics.pendingTasks > 0) {
    insights.push({
      id: "pending-tasks",
      severity: "medium",
      title: `${dashboard.metrics.pendingTasks} items need your attention`,
      detail: "Unread inquiries, pending applications, and open tasks are waiting in your pipeline.",
  ...
  const repeatRate =
    crm.length > 0 ? Math.round((dashboard.metrics.returningClients / Math.max(crm.length, 1)) * 100) : 0;
  if (repeatRate < 15 && crm.length > 10) {
    insights.push({
      id: "repeat-rate",
      severity: "medium",
      title: "Repeat booking rate has room to grow",
      detail: "Launch a referral program or VIP offer for completed clients to increase lifetime value.",
