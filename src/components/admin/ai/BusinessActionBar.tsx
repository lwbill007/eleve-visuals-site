...
const buttonClass = (compact: boolean, type: BusinessAction["type"], className?: string) =>
  cn(
    "relative z-20 inline-flex cursor-pointer items-center justify-center rounded-lg border transition-colors no-underline",
...
export function BusinessActionButton({
  ...
  return (
    <a href={href} className={buttonClass(compact, action.type, className)}>
      {action.label}
    </a>
  );
}
...
export function BusinessActionBar({
  ...
  return (
    <div className={cn("relative z-20 flex flex-wrap gap-2", className)}>
      {actions.map((a) => (
